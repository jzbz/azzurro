...
PROFILE="${AZZURRO_NOTARY_PROFILE:-azzurro-notary}"
...
2. An App Store Connect API key of this app's own, stored as a notarytool
   ...
   your shell history:
       ...
       xcrun notarytool store-credentials azzurro-notary \
...
INPUT="${1:-}"
[ -n "$INPUT" ] || die "usage: $0 <unsigned .zip | Azzurro.app>   (or --setup)"
...
echo "  arches:    $(lipo -archs "$APP/Contents/MacOS/azzurro" 2>/dev/null || echo '?')"
...
# needs none: Slint renders through Metal or the software fallback and compiles
# no code at runtime, it loads no third-party libraries into itself, and it is
# not sandboxed. allow-jit, allow-unsigned-executable-memory and
# disable-library-validation are all cargo-cult here, and each one traded away
# for nothing is a weaker hardened runtime on a program that talks to the
# network.
step "Signing"
...
printf '  xattr -w com.apple.quarantine "0083;00000000;Safari;$(uuidgen)" Azzurro.app\n'
printf '  spctl -a -vvv -t exec Azzurro.app && open Azzurro.app\n'
